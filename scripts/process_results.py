...
flow_cap = model.results.flow_cap.to_dataframe().dropna().reset_index().drop(columns=["carriers"])
flow_out = model.results.flow_out.to_dataframe().dropna().reset_index().drop(columns=["carriers", "timesteps"]).groupby(["nodes", "techs"]).sum().reset_index()

transmission_techs = list(model.inputs.techs.loc[model.inputs.base_tech == "transmission"].values)
...
plot_stacked_bar(flow_out_generation, x="nodes", y="flow_out", stack="techs", color=color, destination=path_plots / f"{name_model}_flow_out.png")
plot_stacked_bar(flow_cap_generation, x="nodes", y="flow_cap", stack="techs", color=color, destination=path_plots / f"{name_model}_flow_cap.png")
